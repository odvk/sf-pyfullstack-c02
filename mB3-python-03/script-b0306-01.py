...
# Допустим, мы собираемся итерироваться по списку наших пользователей, рассчитывать для них
# специальную праздничную скидку, но хотели бы остановиться в какой-то момент.

"""
for i in range(len(user_list)):
    huge_holiday_discount = calculate_complicated_operations(i)
...
# Давайте проверим данные первого попавшегося события от конкретного реферера.
# Мы можем итерироваться по списку событий, пока не встретим первое с верным значением соответствующего ключа.
for event in events:
    if event["referer"] == "https://b24-mu3bxs.bitrix24.shop//katalog/clothes/t-shirts/":
        print(event["item_id"])
        break

# Допустим, мы формируем историю просмотров товаров в срезе их стоимости, складывая в список идентификаторы
# просмотренных/купленных товаров. Но для конкретного реферера мы знаем, что цена была указана неверно,
# поэтому такие события мы хотим на этот раз пропустить, пока цены не будут поправлены.

# Чтобы не исполнять тело цикла дальше, а сразу перейти на следующую итерацию, используется ключевое слово continue:
...
# Мы получим список только тех продуктов, в ценах на которые мы уверены и готовы полагаться на них при анализе данных.
# Тело цикла, где мы инициализируем словарь и кладём его в список,
# не будет исполнено при выполнении условия перед continue.
